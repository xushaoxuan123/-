...
class Flickr8k(Dataset):
    ...
    def load_images(self, im_path):
        ...
        ims = []
        fnames = glob.glob(os.path.join(im_path, 'Images/*.{}'.format('png')))
        fnames += glob.glob(os.path.join(im_path, 'Images/*.{}'.format('jpg')))
        fnames += glob.glob(os.path.join(im_path, 'Images/*.{}'.format('jpeg')))
        texts = []
        masks = []
        ...
        with open(os.path.join(im_path, 'Captions/captions.txt')) as f:
            # header: image,caption
            # format: 1000268201_693b08cb0e.jpg,A child in a pink dress is climbing up a set of stairs in an entry way .
            # each image corresponds to 5 captions
            # save the 5 captions to captions_im
            # after saving the 5 captions, append captions_im to texts, keep reading the captions.txt until all captions are read
            captions_dict = {}
            for line in f.readlines():
                if line.startswith('image,caption'):
                    continue
                # seperate image and caption based on the first comma
                image, caption = line.split(',', 1)
                image = image.split('.')[0]
                if image in captions_dict:
                    captions_dict[image].append(caption.strip())
                else:
                    captions_dict[image] = [caption.strip()]
        ...
        for fname in tqdm(fnames):
            ...
            if 'text' in self.condition_types:
                ...
                # with open(os.path.join(im_path, 'Captions/{}.txt'.format(im_name))) as f:
                #     for line in f.readlines():
                #         captions_im.append(line.strip())
                # texts.append(captions_im)
                texts.append(captions_dict[im_name])
